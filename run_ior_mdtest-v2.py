...
def main():
    ...
    if args.benchmark == "ior":
        ...
        for n_nodes in num_hosts_list:
            ...
            for ppn in ppn_list:
                blocksize_mb = (1.5 * per_node_memory_mb) / ppn
                blocksize_arg = f"{int(blocksize_mb // 1024)}g" if blocksize_mb >= 1024 else f"{int(blocksize_mb)}m"
                print(f"[INFO] Nodes={n_nodes}, PPN={ppn}, blocksize={blocksize_arg} ({blocksize_mb:.2f}MB)")
                ...
                mpi_cmd = [
                    ...
                    '-npernode', str(ppn),
                    'ior', '-t', '1m', '-b', blocksize_arg, '-o', args.workdir, '-vv'
                ]
        ...
        if args.output and scaling_results:
            fieldnames = ['ppn', 'node_count', 'blocksize', 'max_write', 'write_units', 'max_read', 'read_units']
